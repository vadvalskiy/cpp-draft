...
\tcode{sender_in<Expr, env_of_t<Promise>>} is \tcode{true}, and
...
such that \tcode{Sndr} is \tcode{decltype((sndr))}.
If \tcode{sender-for<Sndr, affine_on_t>} is \tcode{false},
...
does not satisfy \tcode{\exposid{infallible-scheduler}<Env>},
then evaluation of
...
type \tcode{\exposid{inline-state}<remove_cvref_t<decltype((\linebreak{}rcvr))>>}
and is potentially-throwing if and only if
...
\pnum
\mandates
\tcode{Sch} satisfies \tcode{\exposid{infallible-scheduler}<env<>>}.
...
otherwise returns
\begin{codeblock}
as_awaitable(affine_on(std::forward<Sender>(sndr)), *this)
\end{codeblock}
\end{itemdescr}
